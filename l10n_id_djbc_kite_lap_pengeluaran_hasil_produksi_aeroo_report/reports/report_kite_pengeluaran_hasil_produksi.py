...
class Parser(report_sxw.rml_parse):
    ...
    def _get_data(self):
        result = []
        obj_data = self.pool.get("l10n_id.lap_kite_pengeluaran_hasil_produksi")
        ...
        if data_ids:
            for no, data in enumerate(obj_data.browse(self.cr, self.uid, data_ids)):
                res = {
                    "no": no + 1,
                    "no_peb": data.no_peb,
                    "tgl_peb": data.tgl_peb,
                    "no_pengeluaran": data.no_pengeluaran,
                    "tgl_pengeluaran": data.tgl_pengeluaran,
                    "penerima": data.penerima.name,
                    "negara_tujuan": data.negara_tujuan.name,
