...
class Parser(report_sxw.rml_parse):
    ...
    def _get_data(self):
        result = []
        obj_data = self.pool.get("l10n_id.lap_kite_pemasukan_bahan_baku")
        ...
        if data_ids:
            for no, data in enumerate(obj_data.browse(self.cr, self.uid, data_ids)):
                res = {
                    "no": no + 1,
                    "jenis_dokumen": data.jenis_dokumen.name,
                    "no_dokumen": data.no_dokumen,
                    "tgl_dokumen": data.tgl_dokumen,
                    "no_seri_barang": data.no_seri_barang.name,
                    "no_penerimaan": data.no_penerimaan,
                    "tgl_penerimaan": data.tgl_penerimaan,
                    "kode_barang": data.kode_barang or "-",
                    "nama_barang": data.nama_barang.name,
                    "satuan": data.satuan.name,
                    "jumlah": data.jumlah,
                    "mata_uang": data.mata_uang.name,
                    "nilai": data.nilai,
                    "gudang": data.gudang.name,
                    "penerima_subkontrak": data.penerima_subkontrak.name,
                    "negara_asal_barang": data.negara_asal_barang.name,
                }
                result.append(res)
        return result
