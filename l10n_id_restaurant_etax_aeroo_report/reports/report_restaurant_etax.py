...
class Parser(report_sxw.rml_parse):
    ...
    def _get_data(self):
        data = []
        obj_pos_order =\
            self.pool.get("pos.order")
        date_start_utc =\
            self._convert_datetime_to_utc(
                self.date_start + ' 00:00:00')
        date_end_utc =\
            self._convert_datetime_to_utc(
        ...
        data_ids =\
            obj_pos_order.search(self.cr, self.uid, criteria)
